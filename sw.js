...
const PRECACHE_URLS = [
  './',
  './index.html',
  './index.html?utm_source=pwa',       /* start_url — cached so PWA launches work offline */
  './index.html?goto=s21&utm_source=shortcut', /* shortcut: Today's Flow */
  './index.html?goto=s8&utm_source=shortcut',  /* shortcut: Reflection   */
  './index.html?goto=s0&utm_source=shortcut',  /* shortcut: Dashboard    */
  './manifest.json',
...
self.addEventListener('activate', function (e) {
  e.waitUntil(
    caches.keys().then(function (keys) {
      ...
      /* Broadcast version to all open tabs after taking control.
         This populates #ubVersion on any tab that was open during update. */
  ...
  e.respondWith(
    caches.match(e.request, { ignoreSearch: true }).then(function (cached) {
      ...
      return fetch(e.request).then(function (response) {
        ...
        caches.open(CACHE_NAME).then(function (cache) {
          /* Normalise cache key: strip query string for same-origin HTML/assets
             so UTM params and goto= params don't create duplicate cache entries */
          var cacheKey = e.request;
          try {
            var u = new URL(e.request.url);
            if (u.origin === self.location.origin &&
                (u.pathname.endsWith('.html') || u.pathname.endsWith('/'))) {
              cacheKey = new Request(u.origin + u.pathname);
            }
          } catch(err) {}
